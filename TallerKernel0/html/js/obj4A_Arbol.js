...
pegaRamaNodo : function(nodox,rama){
	var idP = parseInt(nodox.get('id0'));
	var padre = this.getNodoById(idP);
	var stat = padre.get('stat');
	if (stat = 'FULLA'){
		padre.set('stat','EXPAN');
		padre.set('hijos',new Array());
	} 
	var hijos = padre.get('hijos');
...
showNodosTOL : function(nodo,div,nivel,hijo){
	this.fnShow(nodo,div,nivel);
	var stat = nodo.get('stat');
	if (stat == 'EXPAN'){
		var hijos = nodo.get('hijos');
		...
		for (var i=0;i<n;i++){
			var id = hijos[i];
			var nodox = this.getNodoById(id);
			this.showNodosTOL(nodox,div,nivel+1,i);
		}
	}
},
show : function(div){
...
showTOL : function(div){
	$(div).update();
	var raiz = this.nodos[0];
	this.showNodosTOL(raiz,div,0);

...
function cambiaPassword(pathApp){
	vgSess.pathApp = pathApp;
	var usr = vgSess.hUser.get('usr');
...
//------------------------------------------------------------------- Valida Sesion
function ecoCierraSesion(resp){
	console.log(resp.responseText); // [error : 0]
	window.location = vgSess.homePag;
}

function ajaxCierraSesion (id,path,eco){
	var params = '';
	params += 'id='+id;
	params += '&path='+path;
	var cgi = vgSess.pathCGIs + vgSess.bajaSess;
	retoAjaxPost(params,cgi,eco);
}
function cierraSesion(){
	ajaxCierraSesion(vgSess.idSesion,vgComun.pathApp,ecoCierraSesion)
}
...
function ecoGetFilaUser(resp){
   ...
   vgSess.hUser = filas[0];
   vgSess.ecoPost();
}

function getFilaUser(resp){
   console.log(resp.responseText);
   vgComun.hSesion = str2hash(resp.responseText);
   var rol = vgComun.hSesion.get('rol');
   var usr = vgComun.hSesion.get('usr');
   var id  = vgComun.hSesion.get('id0');
   var sqlStmt = "select * from usuarios where cod='"+usr+"';";
   console.log(sqlStmt);
   ajaxQuerySQLite(id,vgComun.dbaseUsr,sqlStmt,vgComun.pathApp,ecoGetFilaUser);
...
function validaSesion(ecoPost){
	var path = vgComun.pathApp;
	var params = getParamsHTML();
	vgSess.idSesion = params.get('id');
	vgSess.ecoPost= ecoPost;
	vgSess.params = params;
	  if (!vgSess.idSesion) abortaSesion();
	else { ajaxValidaSesion(vgSess.idSesion,path,getFilaUser); return false;}
} 
